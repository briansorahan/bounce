...
test.describe('Audio Commands', () => {
  test('display command should load and visualize WAV file', async () => {
    ...
    const testFile = path.join(__dirname, '../flucoma-core/Resources/AudioFiles/Tremblay-SlideChoirAdd-M.wav');
  ...
  test('play command should load file if not already displayed', async () => {
    ...
    if (!isVisible) {
      throw new Error('Waveform not created when play command used on new file');
    }
