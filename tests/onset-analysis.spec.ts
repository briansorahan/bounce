...
function createTestWavFile(filePath: string, durationSeconds: number = 0.5) {
  ...
  for (let i = 0; i < numSamples; i++) {
    ...
    if (i % 8820 === 0) { // Every 0.2s at 44100 Hz
      value = 0.8;
    } else if (i % 8820 < 441) { // Short decay
...
test.describe('Onset Slice Analysis', () => {
  test('should analyze onset slices and display on waveform', async () => {
    ...
    if (!terminalContent?.includes('Found') || !terminalContent?.includes('onset slices')) {
      throw new Error(`Expected onset slice count message, got: ${terminalContent?.slice(-200)}`);
    }
  ...
  test('should update visualization when running analysis again', async () => {
    ...
    await sendCommand(window, `analyze onset-slice "${testFile}" --threshold 0.5`);
    await window.waitForTimeout(1000);
