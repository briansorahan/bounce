...
@namespace("inst", { summary: "Create and manage sample-based and synthesizer instruments" })
export class InstNamespace {
  /** Namespace summary — used by the globals help() function. */
  readonly description = "Create and manage sample-based and synthesizer instruments";

  private readonly instruments = new Map<string, InstrumentState>();

  constructor(deps: NamespaceDeps) {
    if (deps.onProjectLoad) {
      const originalOnProjectLoad = deps.onProjectLoad;
      deps.onProjectLoad = async () => {
        this.instruments.clear();
        await originalOnProjectLoad();
        await this.restoreInstruments();
      };
    }
    // Restore on initial load
    void this.restoreInstruments();
  }
  ...
  sampler(opts: { name: string; polyphony?: number }): InstrumentResult {
    if (!opts?.name) throw new Error("inst.sampler() requires { name: string }");
    ...
    this.instruments.set(name, state);

    return this.buildInstrumentObject(state);
  }
  ...
  granular(opts: { name: string; polyphony?: number }): InstrumentResult {
    if (!opts?.name) throw new Error("inst.granular() requires { name: string }");
    const name = opts.name;
  ...
  list(): InstrumentListResult {
    const entries = Array.from(this.instruments.values()).map((s) => ({
      name: s.name,
      kind: s.kind,
    ...
    for (const entry of entries) {
      lines.push(`  \x1b[36m${entry.name}\x1b[0m  ${entry.kind} | ${entry.sampleCount} samples`);
    }
  ...
  @param("name", { summary: "Instrument name.", kind: "plain" })
  get(name: string): InstrumentResult | BounceResult {
    const state = this.instruments.get(name);
    if (!state) return new BounceResult(`\x1b[31mInstrument '${name}' not found\x1b[0m`);
    return this.buildInstrumentObject(state);
  }

  // ── Private helpers ───────────────────────────────────────────────────────

  private async restoreInstruments(): Promise<void> {
    ...
    try {
      ...
      for (const record of records) {
        ...
        this.instruments.set(record.name, state);
      }
    } catch (err: unknown) {
      const msg = err instanceof Error ? err.message : String(err);
  ...
  private formatInstrument(state: InstrumentState): string {
  ...
  private instrumentHelp(state: InstrumentState): BounceResult {
  ...
  private buildInstrumentObject(state: InstrumentState): InstrumentResult {
    const display = this.formatInstrument(state);
    ...
    obj.toString = () => this.formatInstrument(state);

    obj.loadSample = Object.assign(
      (note: number, sample: { hash: string }, opts?: { loop?: boolean; loopStart?: number; loopEnd?: number }): BounceResult => {
        if (typeof note !== "number" || note < 0 || note > 127) {
          return new BounceResult("\x1b[31mError: note must be 0–127\x1b[0m");
        }
        if (!sample?.hash) return new BounceResult("\x1b[31mError: sample must have a hash property\x1b[0m");
        const loop = !!opts?.loop;
        const loopStart = opts?.loopStart ?? 0;
        const loopEnd = opts?.loopEnd ?? -1;
        if (loop && loopStart < 0) return new BounceResult("\x1b[31mError: loopStart must be >= 0\x1b[0m");
        if (loop && loopEnd >= 0 && loopEnd <= loopStart) return new BounceResult("\x1b[31mError: loopEnd must be greater than loopStart\x1b[0m");
        ...
        return new BounceResult(`Loaded sample at note ${note}${tags.length > 0 ? ` (${tags.join(" ")})` : ""}`);
      },
      {
    ...
    obj.noteOn = Object.assign(
      (note: number, opts?: { velocity?: number }): BounceResult => {
    ...
    obj.free = Object.assign(
      (): BounceResult => {
        ...
        this.instruments.delete(state.name);
    ...
    obj.set = Object.assign(
      (params: Record<string, number>): BounceResult => {
        const unknown = Object.keys(params).filter(k => !(k in GRANULAR_PARAM_IDS));
        if (unknown.length > 0) return new BounceResult(`\x1b[31mError: unknown params: ${unknown.join(", ")}\x1b[0m`);
    ...
    obj.load = Object.assign(
      (sample: { hash: string }): BounceResult => {
        if (!sample?.hash) return new BounceResult("\x1b[31mError: sample must have a hash property\x1b[0m");
...
/** @deprecated Use `new InstNamespace(deps)` directly. Kept for backward compatibility. */
export function buildInstNamespace(deps: NamespaceDeps): InstNamespace {
  return new InstNamespace(deps);
}

// Re-export commands array for any consumers of the old JSDoc-generated metadata.
export { instCommands as instNamespaceCommands };
