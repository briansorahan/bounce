 ...
 *
 * Core functions (processFile, generateFile) are exported so tests can
 * import and reuse them without triggering the side-effectful main().
 */
 ...
 */
function getNamespaceTag(jsdocText: string): string | undefined {
    ...
    if (ts.isFunctionDeclaration(node) && node.name) {
      ...
      if (jsdoc) {
        ...
        if (namespaceName) {
          ...
          results.push({ namespaceName, commands });
...
export function generateFile(info: NamespaceInfo): string {
  const { namespaceName, commands } = info;
